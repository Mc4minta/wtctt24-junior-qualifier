...
public class BHttpConnectionBase implements HttpConnection, HttpInetConnection {
    ...
    protected InputStream createInputStream(long j2, SessionInputBuffer sessionInputBuffer) {
        ...
        return new ContentLengthInputStream(sessionInputBuffer, j2);
    }

    protected OutputStream createOutputStream(long j2, SessionOutputBuffer sessionOutputBuffer) {
        if (j2 == -2) {
            return new ChunkedOutputStream((int) PKIFailureInfo.wrongIntegrity, sessionOutputBuffer);
        }
        if (j2 == -1) {
            return new IdentityOutputStream(sessionOutputBuffer);
        }
        return new ContentLengthOutputStream(sessionOutputBuffer, j2);
    }
