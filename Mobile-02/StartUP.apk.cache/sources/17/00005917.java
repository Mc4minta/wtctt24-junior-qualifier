...
public class TypeUtils {
    ...
    */
    ...
    public static boolean contains(KotlinType kotlinType, l<UnwrappedType, Boolean> lVar) {
        ...
        return contains(kotlinType, lVar, null);
    }
    ...
    public static List<KotlinType> getImmediateSupertypes(KotlinType kotlinType) {
        ...
        Collection<KotlinType> mo2215getSupertypes = kotlinType.getConstructor().mo2215getSupertypes();
        ArrayList arrayList = new ArrayList(mo2215getSupertypes.size());
        for (KotlinType kotlinType2 : mo2215getSupertypes) {
    ...
    public static TypeParameterDescriptor getTypeParameterDescriptorOrNull(KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo2214getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return (TypeParameterDescriptor) kotlinType.getConstructor().mo2214getDeclarationDescriptor();
        }
        return null;
    }
    ...
    public static boolean hasNullableSuperType(KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo2214getDeclarationDescriptor() instanceof ClassDescriptor) {
            return false;
        }
    ...
    public static boolean isNullableType(KotlinType kotlinType) {
        ...
        if (constructor instanceof IntersectionTypeConstructor) {
            for (KotlinType kotlinType2 : constructor.mo2215getSupertypes()) {
                if (isNullableType(kotlinType2)) {
                    return true;
                }
            }
    ...
    private static boolean contains(KotlinType kotlinType, l<UnwrappedType, Boolean> lVar, SmartSet<KotlinType> smartSet) {
        if (lVar == null) {
            $$$reportNull$$$0(44);
        }
        if (kotlinType == null) {
        ...
        if (noExpectedType(kotlinType)) {
            return lVar.invoke(unwrap).booleanValue();
        }
        if (smartSet == null || !smartSet.contains(kotlinType)) {
            ...
            if (flexibleType == null || !(contains(flexibleType.getLowerBound(), lVar, smartSet) || contains(flexibleType.getUpperBound(), lVar, smartSet))) {
                if ((unwrap instanceof DefinitelyNotNullType) && contains(((DefinitelyNotNullType) unwrap).getOriginal(), lVar, smartSet)) {
                    return true;
                }
                TypeConstructor constructor = kotlinType.getConstructor();
                if (constructor instanceof IntersectionTypeConstructor) {
                    for (KotlinType kotlinType2 : ((IntersectionTypeConstructor) constructor).mo2215getSupertypes()) {
                        if (contains(kotlinType2, lVar, smartSet)) {
                            return true;
                        }
                    }
                    return false;
                ...
                for (TypeProjection typeProjection : kotlinType.getArguments()) {
                    if (!typeProjection.isStarProjection()) {
                        if (contains(typeProjection.getType(), lVar, smartSet)) {
                            return true;
                        }
                    }
                }
    ...
    public static SimpleType makeUnsubstitutedType(TypeConstructor typeConstructor, MemberScope memberScope, l<KotlinTypeRefiner, SimpleType> lVar) {
        ...
        SimpleType simpleTypeWithNonTrivialMemberScope = KotlinTypeFactory.simpleTypeWithNonTrivialMemberScope(Annotations.Companion.getEMPTY(), typeConstructor, getDefaultTypeProjections(typeConstructor.getParameters()), false, memberScope, lVar);
