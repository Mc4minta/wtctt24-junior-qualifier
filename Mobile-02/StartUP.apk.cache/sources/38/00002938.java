...
public final class WalletsModule_ProvidesCompoundFinanceRepositoryFactory implements d<CompoundFinanceRepository> {
    ...
    public WalletsModule_ProvidesCompoundFinanceRepositoryFactory(Provider<CompoundFinanceAPI> provider, Provider<CompoundTokenDAO> provider2) {
        this.compoundFinanceAPIProvider = provider;
        this.compoundTokenDAOProvider = provider2;
    }

    public static WalletsModule_ProvidesCompoundFinanceRepositoryFactory create(Provider<CompoundFinanceAPI> provider, Provider<CompoundTokenDAO> provider2) {
        return new WalletsModule_ProvidesCompoundFinanceRepositoryFactory(provider, provider2);
    }
    ...
    public static CompoundFinanceRepository providesCompoundFinanceRepository(CompoundFinanceAPI compoundFinanceAPI, CompoundTokenDAO compoundTokenDAO) {
        return (CompoundFinanceRepository) h.f(WalletsModule.providesCompoundFinanceRepository(compoundFinanceAPI, compoundTokenDAO));
    }
