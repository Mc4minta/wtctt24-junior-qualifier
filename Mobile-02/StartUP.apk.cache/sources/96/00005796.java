...
public class MemberComparator implements Comparator<DeclarationDescriptor> {
    ...
    private static final DescriptorRenderer RENDERER = DescriptorRenderer.Companion.withOptions(new l<DescriptorRendererOptions, x>() { // from class: kotlin.reflect.jvm.internal.impl.resolve.MemberComparator.1
    ...
    public int compare(DeclarationDescriptor declarationDescriptor, DeclarationDescriptor declarationDescriptor2) {
        ...
        if ((declarationDescriptor instanceof TypeAliasDescriptor) && (declarationDescriptor2 instanceof TypeAliasDescriptor)) {
            ...
            for (int i2 = 0; i2 < Math.min(valueParameters.size(), valueParameters2.size()); i2++) {
                ...
                int compareTo3 = descriptorRenderer3.renderType(valueParameters.get(i2).getType()).compareTo(descriptorRenderer3.renderType(valueParameters2.get(i2).getType()));
                if (compareTo3 != 0) {
                    return compareTo3;
                }
            }
            ...
            for (int i3 = 0; i3 < Math.min(typeParameters.size(), typeParameters2.size()); i3++) {
                ...
                for (int i4 = 0; i4 < upperBounds.size(); i4++) {
                    ...
                    int compareTo4 = descriptorRenderer4.renderType(upperBounds.get(i4)).compareTo(descriptorRenderer4.renderType(upperBounds2.get(i4)));
                    if (compareTo4 != 0) {
                        return compareTo4;
                    }
                }
